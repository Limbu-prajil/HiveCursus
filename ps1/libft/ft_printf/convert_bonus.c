...
/*   Updated: 2024/08/09 17:35:21 by pdietric         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "convert_bonus.h"
#include <stdint.h>
#include "../libft.h"
...
size_t	arglen(t_arg *val, t_conv_spec *cs)
{
	if (ft_strchr("c%", cs->type))
		return (1);
	else if (cs->type == 's' && cs->p >= 0 \
			&& (size_t)cs->p < ft_strlen((char *)val->p))
		return (cs->p);
	else if (cs->type == 's')
...
char	*arg_to_basestr(t_arg val, t_conv_spec *cs)
{
	...
	else if (ft_strchr("p", cs->type) && (val.p == 0) && (cs->p == 0))
		ft_strlcpy(base, "", cs->len_nude + 1);
