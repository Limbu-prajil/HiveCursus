...
void	test_ft_calloc()
{
    ...
    if (ptr1 && ft_memcmp(ptr1, (int[5]){0}, 5 * sizeof(int)) == 0) 
    ...
    int *ptr7 = ft_calloc(1000000, sizeof(int));
    if (ptr7) printf("Large allocation passed.\n");
    free(ptr7);
    
    // Memory Content Check (Zero Initialization)
    int *ptr8 = ft_calloc(10, sizeof(int));
    if (ptr8 && ft_memcmp(ptr8, (int[10]){0}, 10 * sizeof(int)) == 0)
