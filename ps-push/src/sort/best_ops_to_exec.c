...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/21 15:09:09 by plimbu            #+#    #+#             */
/*   Updated: 2025/03/21 15:09:15 by plimbu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rra_rrb_exec(t_data *stacks, int best_depth_in_a, int best_depth_in_b)
{
	while (best_depth_in_a != ft_lstsize(stacks->a)
		&& best_depth_in_b != ft_lstsize(stacks->b))
	{
		rrr(stacks);
		best_depth_in_a++;
		best_depth_in_b++;
...
void	best_ops_to_exec(t_data *stacks, int best_dept_in_a, int best_dept_in_b)
{
	int	sizea;
	int	sizeb;
	...
	if (best_dept_in_a > sizea / 2 && best_dept_in_b > sizeb / 2)
		rra_rrb_exec(stacks, best_dept_in_a, best_dept_in_b);
	else if (best_dept_in_a <= sizea / 2 && best_dept_in_b <= sizeb / 2)
		ra_rb_exec(stacks, best_dept_in_a, best_dept_in_b);
	else if (best_dept_in_a <= sizea / 2 && best_dept_in_b > sizeb / 2)
		ra_rrb_exec(stacks, best_dept_in_a, best_dept_in_b);
	else if (best_dept_in_a > sizea / 2 && best_dept_in_b <= sizeb / 2)
		rra_rb_exec(stacks, best_dept_in_a, best_dept_in_b);
}
